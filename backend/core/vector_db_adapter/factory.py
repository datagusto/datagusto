...
class VectorDatabaseFactory:
    ...
    def __init__(self) -> None:
        ...
        if adapter_name not in ADAPTERS:
            raise ValueError(
                "Vector database is not configured properly. Please check VECTOR_DB_USAGE_TYPE in .env file.",
            )
    ...
    def get_vector_database_join(self) -> "VectorDatabaseBase":
        ...
        endpoint = os.getenv("VECTOR_DB_ENDPOINT", None)
        adapter = ADAPTERS[self.adapter_name]
        return adapter(endpoint=endpoint, class_name=self.join_data_class_name)
