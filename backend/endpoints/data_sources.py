...
def req_get_data_sources(skip: int = 0, limit: int = 100, db: Session = Depends(get_db),
...
def req_get_data_source_by_id(data_source_id: int, current_user: User = Depends(get_current_user),
...
def req_get_columns_in_table(data_source_id: int, table_name: str, current_user: User = Depends(get_current_user),
                             db: Session = Depends(get_db)):
...
def req_create_data_source(data_source: data_source_schema.DataSourceCreate,
                           current_user: User = Depends(get_current_user),
                           db: Session = Depends(get_db)):
...
def req_delete_data_source_by_id(data_source_id: int, current_user: User = Depends(get_current_user),
                                 db: Session = Depends(get_db)):
    # delete metadata
...
def req_create_data_source_from_file(detail: str = Form(...), file: UploadFile = File(...),
